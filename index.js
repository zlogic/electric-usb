...
function createWindow() {
	...
	const allowUsbView = new WebContentsView({ webPreferences: { preload: path.join(__dirname, 'static', 'preload-select-device.js'), devTools: false, transparent: true, partition: 'permissions' } });
	allowUsbView.webContents.loadFile('static/select-device.html');

	let startPageView = new WebContentsView({ webPreferences: {preload: path.join(__dirname, 'static', 'preload-start.js'), devTools: false } });
